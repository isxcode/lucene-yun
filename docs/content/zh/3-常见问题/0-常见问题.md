...
```text
这是因为spark的standalone集群版本，和lucene-yun中spark-min中自带的版本冲突，需要将两边的版本保持一致
```

```bash
cp -r /opt/spark/* /home/ispong/lucene-yun/spark-min/
rm /home/ispong/lucene-yun/spark-min/conf/spark-env.sh
rm /home/ispong/lucene-yun/spark-min/conf/spark-defaults.conf
```
