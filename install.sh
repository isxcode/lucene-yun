...
if [ ! -f "${BASE_PATH}"/lucene-yun-backend/lucene-yun-main/src/main/resources/libprql_java-osx-arm64.dylib ]; then
  curl -ssL https://isxcode.oss-cn-shanghai.aliyuncs.com/zhiqingyun/install/libprql_java-osx-arm64.dylib -o ${BASE_PATH}/lucene-yun-backend/lucene-yun-main/src/main/resources/libprql_java-osx-arm64.dylib
  echo "prql_java-osx-arm64.dylib下载成功"
fi

# prql 二进制文件(linux amd64)
if [ ! -f "${BASE_PATH}"/lucene-yun-backend/lucene-yun-main/src/main/resources/libprql_java-linux64.so ]; then
  curl -ssL https://isxcode.oss-cn-shanghai.aliyuncs.com/zhiqingyun/install/libprql_java-linux64.so -o ${BASE_PATH}/lucene-yun-backend/lucene-yun-main/src/main/resources/libprql_java-linux64.so
